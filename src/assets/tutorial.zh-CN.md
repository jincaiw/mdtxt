...
> 提示：**Live Preview** 是默认写作视图。按 **Ctrl/⌘+/** 进入源码模式，
> 再按一次即可返回。需要并排查看源码与阅读效果时，可使用扩展分栏视图
> （**Ctrl/⌘+\\**）。

打开大纲（底栏的列表图标；Windows/Linux 为 **Ctrl+Shift+1**，macOS 为 **Cmd+Ctrl+1**），下方每一个标题都会变成可点击的目录项。
...
| 功能 | 快捷键 | 说明 |
| --- | --- | --- |
| 快速打开 | `Ctrl+P` / `Cmd+Shift+O` | 按名称寻找文件 |
| 命令面板 | `Ctrl/⌘+Shift+P` | 搜索操作 |
| 文件浏览器 | `Ctrl/⌘+Shift+L` | 浏览当前文件夹 |
| 大纲 | `Ctrl+Shift+1` / `Cmd+Ctrl+1` | 在标题间跳转 |
| 文章 | `Ctrl+Shift+2` / `Cmd+Ctrl+2` | 以扁平列表浏览 Markdown 文件 |
| 文件树 | `Ctrl+Shift+3` / `Cmd+Ctrl+3` | 浏览工作区目录层级 |
