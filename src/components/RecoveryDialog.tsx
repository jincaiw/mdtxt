...
import { latestRecoveryBatch } from "../utils/recoveryModel";
...
interface RecoveryDialogProps {
    entries: RecoveryCandidate[];
    onRestore: (entry: RecoveryCandidate) => void;
    onRestoreAll: (entries: RecoveryCandidate[]) => void;
    onDiscard: (entry: RecoveryCandidate) => void;
}

export function RecoveryDialog({ entries, onRestore, onRestoreAll, onDiscard }: RecoveryDialogProps) {
    const { t, formatDate } = useLocale();
    const firstRef = useRef<HTMLButtonElement>(null);
    const newestBatch = latestRecoveryBatch(entries);
    const hasOlderRecoveryBatch = newestBatch.length < entries.length;
