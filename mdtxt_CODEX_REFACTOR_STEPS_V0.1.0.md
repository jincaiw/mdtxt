...
当前执行采用双泳道：**P6c：焦点与输入安全** 与 **P8b：非破坏冲突路径**。P6a/P6b 只证明最小装饰能够安全挂载且入口被可靠隔离，**不证明** IME、选择、撤销、大文件、平台或默认 Live 已通过；P8a 已建立保存结果哈希闭环，但 P8 仍未证明完整冲突体验或跨平台耐久性。
...
4. P8a 已完成；接着完成 P8b 的比较、重载、保留本地、另存为路径，覆盖活动和后台标签。P8 修改不得恢复 React 全文副本或绕过 `DocumentSessionController`。
