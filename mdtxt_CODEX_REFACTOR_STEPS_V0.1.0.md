...
> 状态：**Final v0.8**（2026-07-15，P6 Beta 与 P8 文件安全并行实施中；本版为后续执行唯一计划）
...
后续：P6 的 Beta 证据闭环 → P7 Widgets；P8 在 P7 期间继续完成平台证据
...
| P8 文件安全、冲突与恢复 | 进行中，未验收 | `20017a3`–`8af307a`；原子替换、修订与哈希防护、可见冲突入口、校验恢复、失败注入和 macOS 恢复/文件系统实测均已落地；以 `docs/audits/p8-file-safety-tracking.md` 为唯一证据表 | 完成 Windows/Linux 恢复与文件系统矩阵，并对每个 post-rename 不确定性提供可见告警 |
...
当前执行采用双泳道：**P6c/P6d：原生输入与受限 Live 性能证据** 与 **P8c/P8d：恢复及耐久性平台验证**。P6a/P6b 只证明最小装饰能够安全挂载且入口被可靠隔离，**不证明** IME、选择、撤销、大文件、平台或默认 Live 已通过；P8a/P8b 已建立哈希闭环与完整冲突体验，但 P8 仍未证明跨平台恢复和文件系统耐久性。
...
4. P8a/P8b 已完成；P8c 先补 Windows/Linux 恢复，P8d 再补目标平台的实际替换、目录同步、符号链接、长路径、UNC 与锁矩阵。目录同步在替换后失败时只允许“成功 + 耐久性告警”的真实语义。P8 修改不得恢复 React 全文副本或绕过 `DocumentSessionController`。
