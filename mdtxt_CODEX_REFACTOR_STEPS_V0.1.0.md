...
| P6 Live Beta | 已验收，保持显式 Beta | macOS Apple 简体拼音、Windows Microsoft Pinyin、Ubuntu Fcitx5 Pinyin 均有原生 WebView 组合/提交与 Source/Live round-trip；1 MiB 输入和 10 MiB Source/受限 Live 有目标平台测量 | 不默认启用；日文 IME 不在 0.1.0 范围 |
| P7 复杂 Widgets | 已验收 | 图像、代码、frontmatter、表格、数学、Mermaid、脚注、callout 八类均交付；CI `29954555501` 原生 WebKit 记录八 Widget、Mermaid、1155 ms 激活与逐字 round-trip | Live 保持显式 Beta；大文档继续使用受限 Live |
...
| B：P8 文件安全 | 保存结果 `mtime + hash` 闭环 → 冲突选择 → 恢复与失败注入 → 文件系统证据 | 已验收；任何后续数据安全回归仍阻断发布且 Live 保持非默认 |
...
| P8c ✓ 跨平台恢复验收 | 副本校验、有限保留、启动发现、恢复为新未保存标签、会话顺序/活动标签/光标行及最新批次隔离 | macOS 隔离 Debug、生产 Windows MSI `taskkill /F`、生产 Ubuntu DEB `SIGKILL` 均通过两草稿恢复且原路径不可能被覆盖 |
| P8d ✓ 耐久性与平台矩阵 | 写、同步、重命名、目录同步失败注入；符号链接、长路径、UNC、锁和替换语义 | 三平台矩阵完成；Windows denied-share 可见，POSIX advisory-lock 限制明确，post-rename 不确定性返回 `durabilityWarning` |
...
因此它被明确限定为 GA 阻塞，不阻塞 v0.1.0 公开预发布。
