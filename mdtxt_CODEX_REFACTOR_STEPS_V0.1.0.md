...
> 状态：**Final v0.7**（2026-07-15，P6 Beta 与 P8 文件安全并行实施中；本版为后续执行唯一计划）
...
后续：P6 验收 → P7 Widgets；P8 在 P7 期间继续完成平台证据
      P7 + P8 均验收 → P9 AI/导出/平台 → P10 发布工程 → P11 发布判定
```
...
| P6 Live Beta | 进行中，未验收 | `9232178` 建立 fixture、round-trip、IME 清单和 1/10 MiB 基准方法；`39b4604` 以 Lezer `syntaxTree` + `StateField<DecorationSet>` 添加只样式化、源码不隐藏的最小展示；`752a41a` 以 Compartment 重配置且不重建 host | 完成焦点/IME/选择/撤销验证、受限 Live 与真实平台证据；显式 Beta 开关已是入口基线，不等于退出 |
| P7 复杂 Widgets | 未开始 | 不提前实现 | 依赖 P6 退出与焦点/降级协议 |
| P8 文件安全、冲突与恢复 | 进行中，未验收 | `20017a3`–`0eb8dab`；原子替换、修订与哈希防护、可见冲突入口、校验恢复和 macOS 恢复实测均已开始落地；以 `docs/audits/p8-file-safety-tracking.md` 为唯一证据表 | 完成前端保存结果哈希闭环、完整非破坏冲突路径、失败注入及三平台文件系统/恢复证据 |
...
6. 保存结果必须由 Rust 返回实际落盘后的修订与内容哈希；前端仅在 `documentId + version` 仍匹配时更新会话的 dirty、mtime、hash 与恢复状态。不得用请求前的值推测保存成功。
...
| 子关卡 | 范围 | 强制验收 |
| --- | --- | --- |
| P8a ✓ 保存结果契约 | `read_file` 返回原始字节哈希；`save_file` 接收预期修订/哈希，并返回实际落盘的 `{ modified, hash }`；显式保存、自动保存、后台保存、关闭保存与另存为统一消费该结果 | 同 mtime 内容变化、过期版本、过期哈希和保存后 hash 更新均由 Rust/会话/hook 回归覆盖；失败保存不清 dirty 或恢复副本 |
| P8b ✓ 外部变化与冲突 | 脏文档不推进已知磁盘修订；提供比较、重载磁盘、保留本地、另存为，覆盖活动和后台标签入口 | 每一种选择均可见且非破坏；旧磁盘字节和本地缓冲区不会被静默覆盖；取消保持可编辑状态 |
| P8c 恢复边界 | 校验、有限保留、启动发现、恢复为新未保存标签、丢弃和失败提示 | 人为终止/重启后可恢复；损坏副本被拒绝；恢复绝不覆盖原路径；三平台记录分别存在 |
| P8d 耐久性与平台矩阵 | 写、同步、重命名、目录同步失败注入；符号链接、长路径、UNC、锁和替换语义 | 每个失败点证明原文件字节与编辑缓冲可存活；macOS、Windows、Ubuntu LTS 逐项记录，平台特例有明确降级文案 |
...
当前执行采用双泳道：**P6c：焦点与输入安全** 与 **P8c/P8d：恢复及耐久性验证**。P6a/P6b 只证明最小装饰能够安全挂载且入口被可靠隔离，**不证明** IME、选择、撤销、大文件、平台或默认 Live 已通过；P8a/P8b 已建立哈希闭环与完整冲突体验，但 P8 仍未证明跨平台恢复和文件系统耐久性。
...
4. P8a/P8b 已完成；接着完成 P8c 的 Windows/Linux 恢复验证以及 P8d 的写、同步、重命名、目录同步失败注入与文件系统矩阵。P8 修改不得恢复 React 全文副本或绕过 `DocumentSessionController`。
5. P6c 以真实 IME、选择、剪贴板、undo/redo、标签切换矩阵为先；P6d 仅在 P6c 无 P0 后记录 1 MiB/10 MiB 原生 WebView 测量。P7 不得提前开始。
6. 每完成一个子关卡，更新对应的 `docs/audits/p6-live-beta-tracking.md` 或 `docs/audits/p8-file-safety-tracking.md`，记录提交、命令、机器/平台、未覆盖原因和回滚点。
