...
> 状态：**Final v0.9**（2026-07-15，按当前代码与证据复评；P6 Beta 与 P8 文件安全并行实施中；本版为后续执行唯一计划）
...
| P8 文件安全、冲突与恢复 | 进行中，未验收 | `20017a3`–`14e8d07`；原子替换、修订与哈希防护、可见冲突入口、校验恢复、失败注入、恢复键跨启动隔离及 macOS 单恢复副本/文件系统实测均已落地；以 `docs/audits/p8-file-safety-tracking.md` 为唯一证据表 | 补齐 AC-007 的多标签/活动标签/位置恢复，再完成 Windows/Linux 恢复与文件系统矩阵，并对每个 post-rename 不确定性提供可见告警 |
| P10a 平台证据基座 | 可并行启动，不计入发布工程完成 | 当前仅有 macOS Debug 证据；Linux Docker 尝试不可计入目标平台通过 | 建立 Windows x64 与 Ubuntu LTS x64 的可复跑构建、安装/启动和矩阵采集入口；不得据此宣称平台功能通过 |
...
| P6c 焦点与输入安全 | 已有统一 `EditFocusResolver` 处理主光标、多选区、composition range、鼠标和查找命中；保持源码可编辑，补齐真实原生 WebView 验证 | 中文/日文 IME、选择、复制粘贴、undo/redo、模式/标签切换手工矩阵无 P0；每个平台未验证项如实记录 |
...
| P8c 恢复边界 | 分两层完成：副本层的校验、有限保留、启动发现、恢复为新未保存标签、丢弃和失败提示；会话层的多标签、活动标签与大致选择/滚动位置恢复 | 单副本恢复不能替代 AC-007；人为终止/重启后必须恢复两标签、活动标签和大致位置；损坏副本被拒绝；恢复绝不覆盖原路径；三平台记录分别存在 |
...
该子阶段只解决“能在真实目标环境重复采集证据”，不改变产品功能、不启用更新器、不制作 GA 声明。先固定 Windows x64 和 Ubuntu LTS x64 的 runner/虚拟机说明、依赖安装、Debug 包构建、安装/启动脚本、日志与截图归档位置；将 P6 的 IME/性能矩阵和 P8 的恢复/文件系统矩阵作为同一证据包的必填附件。Docker 交叉编译、CI 编译成功或非目标发行版容器均不得替代此记录。
...
当前执行采用三泳道：**P6c/P6d：原生输入与受限 Live 性能证据**、**P8c/P8d：恢复及耐久性平台验证**，以及 **P10a：目标平台构建与证据采集基座**。P6a/P6b 只证明最小装饰能够安全挂载且入口被可靠隔离，**不证明** IME、选择、撤销、大文件、平台或默认 Live 已通过；P8a/P8b 已建立哈希闭环与完整冲突体验，现有 P8c 仅证明单恢复副本，仍未证明 AC-007 的会话级恢复和跨平台文件系统耐久性。
...
4. P8a/P8b 已完成；P8c 先补 AC-007 会话级恢复（两标签、活动标签与大致位置）的失败测试和实现，再补三平台恢复记录；P8d 的目标平台替换、目录同步、符号链接、长路径、UNC 与锁矩阵可与 P8c 并行采集。目录同步在替换后失败时只允许“成功 + 耐久性告警”的真实语义。P8 修改不得恢复 React 全文副本或绕过 `DocumentSessionController`。
...
7. P10a 仅维护环境与原始证据索引；当目标平台不可用时记录阻塞原因、最后尝试时间和替代方案，不得以容器、交叉编译或其他 OS 的结果填充 P6/P8 通过单元格。
