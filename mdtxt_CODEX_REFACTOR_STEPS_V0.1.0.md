...
> 状态：**Final v1.0**（2026-07-15，按 `243c22a` 及追踪证据复评；P6 Beta 与 P8 文件安全并行实施中；本版为后续执行唯一计划）
...
| P6 Live Beta | 进行中，未验收 | `9232178`、`39b4604`、`752a41a` 建立最小、可逆的 Lezer/Decoration/Compartment 路径；`d63cb43` 改用 Lezer 识别受限 Live 复杂节点，`2d115f2` 使文档增长后的降级判定随展示快照更新，`6630642` 固定单实例依赖树并记录状态基准 | 完成真实 IME/选择/撤销验证、受限 Live 与原生性能证据；状态基准不含 DOM/WebView/IME 路径，显式 Beta 开关也不等于退出 |
...
| P10a 平台证据基座 | 基座完成，目标平台证据未完成；不计入发布工程完成 | `019c86e` 建立手动三平台 Debug 包和 SHA-256 清单采集，`edf96d7` 对齐 CI 目标，`243c22a` 记录干净 macOS Debug 包；当前尚无 mdtxt 的 Windows/Ubuntu 运行结果 | 获得 mdtxt 专属远程或真实 Windows x64、Ubuntu LTS x64 环境后，执行构建、安装/启动和 P6/P8 矩阵；不得据此宣称平台功能通过 |
...
该子阶段只解决“能在真实目标环境重复采集证据”，不改变产品功能、不启用更新器、不制作 GA 声明。工作流已提供 Windows x64、Ubuntu 24.04 x64 与 macOS 14 ARM64 的 Debug 包和 SHA-256 清单；下一步是在 mdtxt 专属远程/ref 或人工目标环境实际执行安装/启动，并将 P6 的 IME/性能矩阵和 P8 的恢复/文件系统矩阵作为同一证据包的必填附件。当前 `origin` 是上游 Paperling，不能触发或引用其发布结果作为 mdtxt 证据。Docker 交叉编译、CI 编译成功或非目标发行版容器均不得替代此记录。
...
| 3 | 获得 mdtxt 专属远程/ref，或可交付的目标平台人工证据 | 触发/归档 P10a Debug 包、清单、安装启动日志和对应矩阵链接 | 上游 Paperling remote、交叉编译或容器结果不得替代；没有专属证据则保持 Pending |
