...
> 状态：**Final v0.4**（2026-07-15，再评估定稿）
...
后续：P5 编辑器分层 → P6 Live Beta → P7 Widgets
      P4 完成后并行推进 P8 文件安全 → P9 AI/导出/平台 → P10 发布工程 → P11 发布判定
```

唯一可接受的主路径仍是：**先消除 React 与 CodeMirror 的双全文状态，再拆分编辑器，再引入 Live 与 Widgets**。不得以“模型已建立”代替状态迁移完成；当前 `DocumentSession`、版本令牌与按标签模式已存在，显式保存与活动标签自动保存已采用版本化读取；但 `App.tsx` 仍保留活动全文 React 状态，因此 P4 尚未退出。
...
| P4 文档会话 | 进行中 | ADR、纯模型、版本防护、会话控制器、每标签 `EditorState`；`8a087ec`、`a8c782c` 已迁移显式保存与活动自动保存 | 清除按键级全文 React 主状态及 tab 正文副本 |
...
| P4d 迁移收口 | 按下列 P4d-1 至 P4d-4 的顺序消除活动全文状态、`docSwapId` 式全文替换及 tab 内容副本；预览/保存/AI 读取版本化快照 | 输入路径没有按键级全文 React 更新；Source/Split/Reader、自动保存、外部变化、AI 审阅回归通过 |
...
下一项工作固定为 **P4d-2：后台会话操作**。将后台标签自动保存、外部变化、关闭标签/窗口和恢复改为从会话控制器按需读取；随后执行 P4d-3 展示投影与 P4d-4 删除遗留副本。不在这些提交拆分其余编辑器模块或实现 Live。P4d-1 至 P4d-4 全部通过后，才进入 P5。
