...
| 子关卡 | 范围 | 强制验收 |
| --- | --- | --- |
| P8a 保存结果契约（进行中） | `read_file` 返回原始字节哈希；`save_file` 接收预期修订/哈希，并返回实际落盘的 `{ modified, hash }`；显式保存、自动保存、后台保存、关闭保存与另存为统一消费该结果 | 同 mtime 内容变化、过期版本、过期哈希和保存后 hash 更新均有 Rust/会话/hook 回归；失败保存不清 dirty 或恢复副本 |
...
当前执行采用双泳道：**P6c：焦点与输入安全** 与 **P8a：保存结果哈希闭环**。P6a/P6b 只证明最小装饰能够安全挂载且入口被可靠隔离，**不证明** IME、选择、撤销、大文件、平台或默认 Live 已通过；P8 已有原子写入与恢复实现也**不证明**所有保存路径已经消费落盘后的哈希，或跨平台耐久性已通过。
...
4. P8a 先完成 Rust/会话/前端的 `{ modified, hash }` 原子结果链和回归；随后做 P8b 的完整冲突路径。P8 修改不得恢复 React 全文副本或绕过 `DocumentSessionController`。
