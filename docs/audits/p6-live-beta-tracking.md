...
| Native large-document and input smoke | [`CI #29632299665`](https://github.com/jincaiw/mdtxt/actions/runs/29632299665), commit `007843b`; Ubuntu 24.04 Linux WebKit | Ubuntu passed: six native tests, 10 MiB Source open 148 ms, restricted Live 16 ms, and 1 MiB W3C-key input processing p50/p95 0/0 ms (max 1 ms) with the exact suffix preserved. Windows failed before session creation. Follow-up [`CI #29632824144`](https://github.com/jincaiw/mdtxt/actions/runs/29632824144) proved all three candidate roots were watched but no `DevToolsActivePort` was created, narrowing the harness fault to remote debugging not being enabled. The next follow-up supplies `--remote-debugging-port=0` through both EdgeDriver `webviewOptions.additionalBrowserArguments` and the test-only WebView2 environment, and remains pending until CI proves the repair. |
