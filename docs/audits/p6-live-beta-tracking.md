...
| Native large-document and input smoke | [`CI #29632299665`](https://github.com/jincaiw/mdtxt/actions/runs/29632299665), commit `007843b`; Ubuntu 24.04 Linux WebKit | Ubuntu passed: six native tests, 10 MiB Source open 148 ms, restricted Live 16 ms, and 1 MiB W3C-key input processing p50/p95 0/0 ms (max 1 ms) with the exact suffix preserved. Windows failed before session creation. Follow-ups [`CI #29632824144`](https://github.com/jincaiw/mdtxt/actions/runs/29632824144) and [`CI #29633387810`](https://github.com/jincaiw/mdtxt/actions/runs/29633387810) proved that watching all candidate roots and supplying `--remote-debugging-port=0` through both WebDriver capabilities and the test-only WebView2 environment did not create a usable launch session. Cancelled diagnostic [`CI #29634011698`](https://github.com/jincaiw/mdtxt/actions/runs/29634011698) restored direct `tauri-driver` and again reproduced EdgeDriver's 60-second `DevToolsActivePort file doesn't exist`, while all non-native-Windows jobs and Ubuntu's six native tests passed. The Windows harness now uses Microsoft's supported attach flow: launch the real app with a fixed remote-debugging port, then connect EdgeDriver through `debuggerAddress`. Current-CI proof remains pending. |
