...
| Crash recovery | `src-tauri/src/recovery.rs` writes app-data recovery entries atomically with SHA-256 validation and seven-day retention; each entry now carries a per-launch batch id, tab order, active-tab marker and cursor line; startup lists verified entries in `RecoveryDialog` | Restore creates new unsaved tabs and cannot overwrite disk paths or collide with a prior launch's recovery key. `Restore all` restores verified entries in original tab order, selects the recorded active tab, and returns to its saved line; discard stays visible for retry if native deletion fails, while a failed post-restore cleanup is reported as a possible next-launch repeat. The old macOS Debug smoke proves one draft only; the new two-tab force-terminate/relaunch path remains native-unverified |
...
| Frontend release gates | `bun run test && bun run build && bun run release:check` | Passed: 47 files / 320 tests, production build, and preflight (`mdtxt` `0.1.0`; 439 Chinese keys / 98 source files; 0 direct user-copy literals) |
...
| Recovery session placement | `bun run test -- src/utils/recoveryModel.test.ts src/components/RecoveryDialog.test.tsx` | Passed: 5 tests cover original tab ordering despite write-completion order, recorded active-tab selection, legacy-entry fallback, and the explicit Restore all action |
...
1. Perform the macOS AC-007 native smoke: two drafts, their content, active tab and approximate cursor/scroll position after forced termination; record it separately from recovery-entry integrity.
