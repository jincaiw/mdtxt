...
产品所有者已授权在 `jincaiw/mdtxt` 发布 v0.2.1 正式公开包。不得复用上游
...
7. 推送带注释的 `v0.2.1` 标签；metadata job 只创建一个 draft Release，
...
v0.2.1 虽为正式版，仍缺少三平台签名/公证；不得将未签名状态描述为受信任
安装。启用更新器仍须另行验证 mdtxt 自有 HTTPS endpoint 与签名密钥。v0.2.1
