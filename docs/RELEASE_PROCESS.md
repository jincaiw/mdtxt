...
产品所有者已授权在 `jincaiw/mdtxt` 发布 v0.2.0 公开预发布包。不得复用上游
...
7. 推送带注释的 `v0.2.0` 标签；metadata job 只创建一个 draft prerelease，
...
签名密钥。v0.2.0 不创建 winget/Scoop 清单，也不生成 updater JSON。
